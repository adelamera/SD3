...
public class ApacheHttpSubmission {
	...
	// save
	public static void saveSubmission(Long studentId, Long assignmentId, Submission submission) {
		try {
			Gson gson = new Gson();
			DefaultHttpClient httpClient = new DefaultHttpClient();
	...
	// update
	public static void gradeSubmission(Long id, int grade) {
		try {
			DefaultHttpClient httpClient = new DefaultHttpClient();
	...
	// delete
	public static void deleteSubmission(Long id) {
		try {
			DefaultHttpClient httpClient = new DefaultHttpClient();
	...
	public static List<Submission> getAllSubmissionsOfStudent(Long studentId) {
		try {
			DefaultHttpClient httpClient = new DefaultHttpClient();
			HttpGet getRequest = new HttpGet("http://localhost:8080/submissions/students/id?studentId=" + String.valueOf(studentId));
			getRequest.addHeader("accept", "application/json");
			HttpResponse response = httpClient.execute(getRequest);
			BufferedReader br = new BufferedReader(new InputStreamReader((response.getEntity().getContent())));
			String output = br.readLine();
